...
class TeamService {
    async populateTeamUserData(team) {
        try {
            const ownerData = await (0, userClient_1.fetchUserData)(team.owner.toString());
            const membersData = await Promise.all(team.members.map(async (memberId) => {
                const userData = await (0, userClient_1.fetchUserData)(memberId.toString());
    ...
    async getUserTeams(userId) {
        try {
            const teams = await models_1.Team.find({ members: userId })
                ...
                .sort({ createdAt: -1 })
                .lean();
            const teamsWithUserData = await Promise.all(teams.map((team) => this.populateTeamUserData(team)));
    ...
    async getTeamById(teamId, userId) {
        try {
            const team = await models_1.Team.findById(teamId).populate('channels').lean();
            if (!team)
    ...
    async updateTeam(teamId, userId, data) {
        try {
            ...
            if (team.owner.toString() !== userId)
                throw libs_1.Errors.forbidden('Only the team owner can update the team');
    ...
    async deleteTeam(teamId, userId) {
        try {
            ...
            if (!team)
                throw libs_1.Errors.notFound('Team not found');
            if (team.owner.toString() !== userId)
                throw libs_1.Errors.forbidden('Only the team owner can delete the team');
    ...
    async addMember(teamId, userId, memberUserId) {
        try {
            ...
            await team.save();
            await models_1.Channel.updateMany({ teamId, isPrivate: false }, { $addToSet: { members: new mongoose_1.Types.ObjectId(memberUserId) } });
            const updatedTeam = await models_1.Team.findById(teamId)
                .populate('channels')
                .lean();
            const teamWithUserData = await this.populateTeamUserData(updatedTeam);
    ...
    async removeMember(teamId, userId, memberUserId) {
        try {
            ...
            await team.save();
            await models_1.Channel.updateMany({ teamId }, { $pull: { members: memberUserId } });
            const updatedTeam = await models_1.Team.findById(teamId)
                .populate('channels')
                .lean();
            const teamWithUserData = await this.populateTeamUserData(updatedTeam);
    ...
    async inviteMemberByEmail(teamId, userId, email) {
        try {
            ...
            await team.save();
            await models_1.Channel.updateMany({ teamId, isPrivate: false }, { $addToSet: { members: memberUserId } });
            const updatedTeam = await models_1.Team.findById(teamId)
                .populate('channels')
                .lean();
            const teamWithUserData = await this.populateTeamUserData(updatedTeam);
