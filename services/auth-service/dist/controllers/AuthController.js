...
const login = async (req, res, _next) => {
  try {
    ...
    const isPasswordValid = await (0, password_1.comparePassword)(
      password,
      user.password
    );
    if (!isPasswordValid) {
    ...
    libs_1.logger.info('User logged in', {
      userId: user._id,
      email: user.email,
    });
    (0, libs_1.success)(
...
const refresh = async (req, res, _next) => {
  try {
    ...
    const { accessToken, refreshToken: newRefreshToken } = (0,
    token_1.generateTokens)(user._id.toString(), user.email, user.role);
...
const verifyEmail = async (req, res, _next) => {
  try {
    ...
    await user.save();
    libs_1.logger.info('Email verified', {
      userId: user._id,
      email: user.email,
    });
