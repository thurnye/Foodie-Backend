...
class CookbookService {
  ...
  /**
   * Add extra page to cookbook (blank page or template)
   */
  async addExtraPage(
    cookbookId: string,
    userId: string,
    ...
    try {
      ...
      // Check if user is the author
      if (cookbook.author.toString() !== userId) {
        throw Errors.forbidden('You can only add pages to your own cookbooks');
      }
      ...
      logger.info('Extra page added to cookbook', {
        cookbookId,
        userId,
        pageId,
        pageType: pageData.pageType,
      ...
      logger.error('Error adding extra page to cookbook', { error, cookbookId });
      throw error;
    }
  }
  ...
  async updateExtraPage(
    ...
    try {
      ...
      // Check if user is the author
      if (cookbook.author.toString() !== userId) {
        throw Errors.forbidden('You can only update pages in your own cookbooks');
      }
      ...
      if (pageIndex === undefined || pageIndex === -1) {
        throw Errors.notFound('Extra page not found in cookbook');
      }
      ...
      logger.info('Extra page updated in cookbook', { cookbookId, userId, pageId });

      return cookbook;
    } catch (error) {
      logger.error('Error updating extra page in cookbook', { error, cookbookId, pageId });
      throw error;
    }
  }
   ...
   */
  async deleteExtraPage(cookbookId: string, pageId: string, userId: string): Promise<ICookbook> {
    try {
      const cookbook = await Cookbook.findOne({
        _id: cookbookId,
        isActive: true,
      ...
      // Check if user is the author
      if (cookbook.author.toString() !== userId) {
        throw Errors.forbidden('You can only delete pages from your own cookbooks');
      }

      // Find extra page index
      const pageIndex = cookbook.extraPages?.findIndex(p => p.pageId === pageId);
      ...
      logger.info('Extra page deleted from cookbook', { cookbookId, userId, pageId });

      return cookbook;
    } catch (error) {
      logger.error('Error deleting extra page from cookbook', { error, cookbookId, pageId });
      throw error;
    }
  }
}
