...
class CookbookService {
    ...
    async addExtraPage(cookbookId, userId, pageData) {
        try {
            const cookbook = await Cookbook_1.default.findOne({
                _id: cookbookId,
                isActive: true,
            ...
            if (cookbook.author.toString() !== userId) {
                throw libs_1.Errors.forbidden('You can only add pages to your own cookbooks');
            }
            const pageId = `${pageData.section}-${pageData.pageType}-${Date.now()}`;
            const newExtraPage = {
                pageId,
            ...
            if (!cookbook.extraPages) {
                cookbook.extraPages = [];
            }
            cookbook.extraPages.push(newExtraPage);
            await cookbook.save();
            libs_1.logger.info('Extra page added to cookbook', {
                cookbookId,
                userId,
                pageId,
                pageType: pageData.pageType,
            ...
            libs_1.logger.error('Error adding extra page to cookbook', { error, cookbookId });
            throw error;
        }
    }
    async updateExtraPage(cookbookId, pageId, userId, updates) {
        try {
            const cookbook = await Cookbook_1.default.findOne({
                _id: cookbookId,
                isActive: true,
            ...
            if (cookbook.author.toString() !== userId) {
                throw libs_1.Errors.forbidden('You can only update pages in your own cookbooks');
            }
            const pageIndex = cookbook.extraPages?.findIndex(p => p.pageId === pageId);
            if (pageIndex === undefined || pageIndex === -1) {
                throw libs_1.Errors.notFound('Extra page not found in cookbook');
            }
            if (updates.title !== undefined) {
                cookbook.extraPages[pageIndex].title = updates.title;
            }
            if (updates.position !== undefined) {
                cookbook.extraPages[pageIndex].position = updates.position;
            }
            await cookbook.save();
            libs_1.logger.info('Extra page updated in cookbook', { cookbookId, userId, pageId });
            return cookbook;
        }
        catch (error) {
            libs_1.logger.error('Error updating extra page in cookbook', { error, cookbookId, pageId });
            throw error;
        }
    }
    async deleteExtraPage(cookbookId, pageId, userId) {
        try {
            const cookbook = await Cookbook_1.default.findOne({
                _id: cookbookId,
                isActive: true,
            ...
            if (cookbook.author.toString() !== userId) {
                throw libs_1.Errors.forbidden('You can only delete pages from your own cookbooks');
            }
            const pageIndex = cookbook.extraPages?.findIndex(p => p.pageId === pageId);
            if (pageIndex === undefined || pageIndex === -1) {
                throw libs_1.Errors.notFound('Extra page not found in cookbook');
            }
            cookbook.extraPages?.splice(pageIndex, 1);
            await cookbook.save();
            libs_1.logger.info('Extra page deleted from cookbook', { cookbookId, userId, pageId });
            return cookbook;
        }
        catch (error) {
            libs_1.logger.error('Error deleting extra page from cookbook', { error, cookbookId, pageId });
            throw error;
        }
    }
}
